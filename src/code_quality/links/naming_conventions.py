            ...
            for root, _, files in os.walk(dir_path):
                for file in files:
                    ...
                    if not self.module_pattern.match(module_name):
                        issues.append(f"Module name '{module_name}' in {file_path} does not follow snake_case convention")
    ...
    def _check_file_content(self, content: str, file_path: str) -> List[str]:
        ...
        for match in class_pattern.finditer(content):
            ...
            if not self.class_pattern.match(class_name):
                issues.append(f"Class name '{class_name}' in {file_path} does not follow PascalCase convention")
        ...
        for match in function_pattern.finditer(content):
            ...
            if not self.function_pattern.match(function_name):
                issues.append(f"Function name '{function_name}' in {file_path} does not follow snake_case convention")
        ...
        for match in variable_pattern.finditer(content):
            ...
            if (variable_name.startswith('__') or 
                variable_name == 'self' or 
                variable_name == 'cls' or
                variable_name in ['import', 'from', 'as', 'class', 'def']):
                continue
            
            # Check if it's a constant (all uppercase)
            if variable_name.isupper():
                if not self.constant_pattern.match(variable_name):
                    issues.append(f"Constant '{variable_name}' in {file_path} does not follow UPPER_CASE convention")
            # Otherwise it should be a regular variable
            elif not self.variable_pattern.match(variable_name):
                issues.append(f"Variable '{variable_name}' in {file_path} does not follow snake_case convention")
